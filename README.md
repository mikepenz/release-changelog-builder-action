...
> [!WARNING]  
> When includeOnlyPaths is set on GitHub, the action performs one additional API request per commit in the compared range to inspect changed files. On large ranges, this can be slow and may approach API rate limits. Prefer narrowing the from/to range when possible.
...
| `includeOnlyPaths`        | List of path patterns to include. Provide as multiline input (one per line). Only commits that touched these paths will be included in the changelog. Useful for monorepo setups where you want to filter commits by their affected paths. On GitHub, enabling this flag triggers one additional API request per commit in the compared range to inspect changed files; for large ranges this can be slow and may approach API rate limits. Consider narrowing the tag/ref range. Note: summary counts (files/lines) reflect the overall diff, while the commit list is path-filtered. |
